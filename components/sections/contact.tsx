"use client";

import { motion } from "framer-motion";
import { Download, Mail } from "lucide-react";
import { site } from "@/data/site";

const GitHubIcon = ({ className }: { className?: string }) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    className={className}
    xmlns="http://www.w3.org/2000/svg"
    fill="currentColor"
  >
    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
  </svg>
);

const LinkedInIcon = ({ className }: { className?: string }) => (
  <svg
    role="img"
    viewBox="0 0 24 24"
    className={className}
    xmlns="http://www.w3.org/2000/svg"
    fill="currentColor"
  >
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
  </svg>
);

const contactLinks = [
  {
    label: "Email",
    href: site.links.email ? `mailto:${site.links.email}` : "#",
    icon: Mail,
    disabled: !site.links.email,
  },
  {
    label: "GitHub",
    href: site.links.github || "#",
    icon: GitHubIcon,
    disabled: !site.links.github,
  },
  {
    label: "LinkedIn",
    href: site.links.linkedin || "#",
    icon: LinkedInIcon,
    disabled: !site.links.linkedin,
  },
  {
    label: "Resume",
    href: site.links.resume || "#",
    icon: Download,
    disabled: !site.links.resume,
  },
];

export default function Contact() {
  return (
    <section
      id="contact"
      className="relative bg-transparent px-6 py-20 sm:py-24 lg:px-8"
    >
      <motion.div
        initial={{ opacity: 0, y: 18 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.5 }}
        className="
          mx-auto
          max-w-6xl
          overflow-hidden
          rounded-3xl
          border
          border-white/15
          bg-white/[0.05]
          p-6
          shadow-2xl
          shadow-black/30
          backdrop-blur-md
          sm:p-10
        "
      >
        <div className="relative">
          <div className="absolute -right-24 -top-24 -z-10 size-64 rounded-full bg-cyan-500/10 blur-3xl" />
          <div className="absolute -bottom-24 -left-24 -z-10 size-64 rounded-full bg-violet-500/10 blur-3xl" />

          <p className="font-mono text-sm font-semibold uppercase tracking-[0.22em] text-cyan-400">
            Contact
          </p>

          <div className="mt-6 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            {/* Left Side */}
            <div>
              <h2 className="font-display text-4xl font-semibold tracking-tight text-white sm:text-5xl">
                Available for Job/Internship Opportunities
              </h2>

              <p className="mt-6 max-w-2xl text-lg leading-8 text-white/75">
                Open to Software Engineering, Full-Stack Development,
                Data Science, and AI/ML internship opportunities.
              </p>
            </div>

            {/* Right Side */}
            <div className="grid gap-4">
              {contactLinks.map((link) => {
                const Icon = link.icon;

                const isResume = link.label === "Resume";

                return (
                  <a
                    key={link.label}
                    href={link.href}
                    aria-disabled={link.disabled}
                    target={link.label === "Email" ? undefined : "_blank"}
                    rel={link.label === "Email" ? undefined : "noreferrer"}
                    className={`
                      inline-flex items-center justify-center
                      rounded-full
                      px-6 py-4
                      text-base font-semibold
                      transition-all duration-300
                      ${
                        isResume
                          ? `
                          bg-cyan-400
                          text-slate-950
                          hover:bg-cyan-300
                          hover:scale-[1.02]
                        `
                          : `
                          border border-white/15
                          bg-white/[0.04]
                          text-white
                          hover:border-cyan-400/30
                          hover:bg-cyan-400/10
                        `
                      }
                      aria-disabled:pointer-events-none
                      aria-disabled:opacity-50
                    `}
                  >
                    <Icon
                      className={`mr-3 size-5 ${
                        isResume ? "" : "text-cyan-300"
                      }`}
                    />
                    {link.label}
                  </a>
                );
              })}
            </div>
          </div>
        </div>
      </motion.div>
    </section>
  );
}